...
export default function BadgeGenerator() {
  ...
  const handleDownload = () => {
    ...
    logoImg.onload = () => {
      ...
      link.download = `${generated.id}_badge-${generated.norm.replace(/[^a-z0-9]/gi, "_")}.png`;
  ...
  const handleDownloadHTML = () => {
    ...
    const svgLogoInline = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 102.19 97.1" width="115" height="110">
      <path d="M95.06,34.06l-13.91,14.13-3.51,3.58c-1.59,14.57-13.96,25.92-29,25.92-16.11,0-29.17-13.04-29.17-29.12,0-2.52.31-4.96.92-7.29.01-.01.01-.02.01-.03.75-2.92,1.94-5.67,3.52-8.15,5.15-8.21,14.29-13.67,24.72-13.67,6.83,0,13.11,2.33,18.08,6.25l.06-.06,13.6-13.82C71.87,4.45,60.77,0,48.64,0,21.82,0,0,21.78,0,48.56s21.82,48.54,48.64,48.54,48.64-21.77,48.64-48.54c0-5.05-.78-9.92-2.23-14.5Z" fill="#383838"/>
      <path d="M99.9,21.92l-6.81,6.93-15.55,15.8-20.79,21.16c-1.51,1.54-3.5,2.36-5.63,2.36s-4.14-.83-5.65-2.36l-17.74-18.06c-3.07-3.12-3.07-8.21,0-11.33,1.51-1.53,3.5-2.37,5.63-2.37s4.14.85,5.65,2.37l12.11,12.32,19.27-19.59,13.64-13.86,4.61-4.7c1.51-1.52,3.5-2.36,5.63-2.36,1.07,0,2.12.2,3.05.61.96.4,1.83.99,2.58,1.76.08.08.15.15.21.23,2.83,3.13,2.78,8.05-.21,11.09Z" fill="#2549d3"/>
      <path d="M11.69,44.75l-.8-.14-2.21-2.36-.32,1.77-1.08-.19.61-3.42.8.14,2.21,2.36.35-1.95,1.08.19-.64,3.6Z" fill="#fff"/>
      <path d="M29.45,86.84c-.67-.35-1.18-.82-1.54-1.41-.36-.59-.54-1.24-.54-1.97,0-.72.2-1.47.61-2.24.41-.78.91-1.37,1.51-1.78s1.24-.63,1.93-.66c.68-.04,1.36.12,2.03.48.67.35,1.19.82,1.55,1.4.36.58.54,1.23.54,1.96,0,.72-.21,1.48-.62,2.26-.41.77-.91,1.36-1.51,1.77-.6.41-1.24.64-1.93.67-.68.03-1.36-.12-2.03-.48ZM30.24,85.36c.36.19.72.26,1.09.21.37-.05.72-.21,1.05-.48.33-.27.62-.63.86-1.09.25-.47.39-.91.42-1.34.03-.42-.03-.8-.2-1.13-.17-.33-.43-.59-.8-.78-.36-.19-.72-.26-1.09-.21-.37.05-.72.2-1.05.47-.33.27-.62.63-.87,1.1-.24.46-.38.9-.41,1.33-.03.42.03.8.2,1.14.17.33.44.6.8.79Z" fill="#fff"/>
...
<head>
  ...
  <title>Zertifikat – ${generated.system} ${generated.norm}</title>
  <style>
    body { margin: 0; padding: 32px; font-family: 'Segoe UI', Arial, sans-serif; background: #f8faff; display: flex; justify-content: center; }
    .badge-wrapper { display: inline-block; }
    .badge {
      display: inline-flex; align-items: center; gap: 22px;
      background: linear-gradient(135deg, #f0f4ff 0%, #eaeefc 100%);
      border-radius: 22px; padding: 22px 32px 22px 22px;
      box-shadow: 0 2px 20px rgba(37,73,211,0.08);
      border: 1.5px solid rgba(37,73,211,0.12);
      text-decoration: none;
    }
    .badge:hover { box-shadow: 0 4px 28px rgba(37,73,211,0.18); transform: translateY(-1px); transition: all 0.2s; }
    .badge-text { display: flex; flex-direction: column; }
    .system { font-size: 1.1rem; font-weight: 700; color: #1e293b; margin-bottom: 2px; }
    .norm { font-size: 1.25rem; font-weight: 800; color: #2549d3; margin-bottom: 14px; }
    .info { font-size: 0.9rem; font-weight: 700; color: #1e293b; margin-bottom: 3px; }
    .verify-link {
      display: inline-flex; align-items: center; gap: 5px; margin-top: 12px;
      font-size: 0.78rem; font-weight: 600; color: #2549d3;
...
<body>
  <div class="badge-wrapper">
    <a class="badge" href="${verifyUrl}" target="_blank" rel="noopener">
      ${svgLogoInline}
      <div class="badge-text">
        <span class="system">${generated.system}</span>
        <span class="norm">${generated.norm}</span>
        <span class="info">Gültig bis: ${generated.validUntil}</span>
        <span class="info">ID ${generated.id}</span>
      </div>
    </a>
    <br/>
    <a class="verify-link" href="${verifyUrl}" target="_blank" rel="noopener">
    ...
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.download = `${generated.id}_badge-${generated.norm.replace(/[^a-z0-9]/gi, "_")}.html`;
    link.href = url;
    link.click();
    URL.revokeObjectURL(url);
  };
    ...
    <div style={{
      ...
      <div style={{ display: "flex", gap: "12px", marginTop: "20px", flexWrap: "wrap", justifyContent: "center" }}>
        <button
          ...
          style={{
            padding: "13px 24px", background: "white", color: "#2549d3",
            border: "2px solid #2549d3", borderRadius: "10px", fontSize: "0.9rem",
        ...
        <button
          ...
          style={{
            padding: "13px 24px", background: "#2549d3", color: "white",
            border: "2px solid #2549d3", borderRadius: "10px", fontSize: "0.9rem",
            fontWeight: "700", cursor: "pointer", fontFamily: "inherit",
            display: "flex", alignItems: "center", gap: "8px",
            boxShadow: "0 4px 14px rgba(37,73,211,0.3)",
          }}
          onMouseEnter={e => { e.currentTarget.style.background = "#1d3eb8"; e.currentTarget.style.borderColor = "#1d3eb8"; }}
          onMouseLeave={e => { e.currentTarget.style.background = "#2549d3"; e.currentTarget.style.borderColor = "#2549d3"; }}
        >
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/>
            <polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/>
          </svg>
          HTML mit Verifikationslink
        </button>
      </div>
    </div>
  );
